...
export function AliasSelector() {
  const { setAlias } = useUserProfile();
  const { lang, setLang } = useLanguage();
  const [step, setStep] = useState<"lang" | "alias">("lang");
  ...
  const loadAliases = async (l: Language) => {
    ...
    const list = await generateAliases(l).catch(
      () => (ALIAS_FALLBACKS[l] ?? ["Voz Serena", "Marea Quieta", "Eco Profundo", "Luz Tardía", "Raíz Firme"])
    );
    setAliases(list);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background">
      ...
      <AnimatePresence mode="wait">
        {step === "lang" ? (
          ...
          >
            <p className="text-xs uppercase tracking-[0.22em] text-primary/90 text-center">
              People4People
            </p>
            <h1 className="mt-4 font-display text-4xl text-gradient text-center leading-tight">
              Elige tu idioma /<br />Choose your language
            </h1>
            <div className="mt-10 grid grid-cols-2 gap-3">
              {LANGUAGES.map((l) => (
                <button
                  ...
                  className="rounded-2xl border border-border/60 bg-surface hover:border-primary/40 hover:bg-surface-elevated px-5 py-6 text-left transition group"
                >
                  <span className="text-3xl">{l.flag}</span>
                  <p className="mt-3 font-display text-xl group-hover:text-foreground transition">{l.name}</p>
                </button>
              ))}
            </div>
          </motion.div>
